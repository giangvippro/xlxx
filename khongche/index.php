...
function paginates_3() {
    $conn= mysqli_connect("localhost","root","","data_menu");
    $categories= mysqli_query($conn,"SELECT * FROM movies where state='uncen' ORDER BY id");
...
$categories = mysqli_query($conn, "SELECT * FROM movies where state='uncen' limit $start,$limit");
?>
<!DOCTYPE html>
<html lang="vi">
<!-- head -->
...
<head>
    <meta charset="UTF-8">
    <base href="http://localhost/xlxx/khongche">
    <title>Không che - XLXX</title>
    ...
    <style>
        ...
        .nav-menu .row li.col:nth-of-type(3) {
            color: #ff0000 !important;
        }
        .nav-menu .row li.col:not(:nth-of-type(3)) {
            color: #fff !important;
        }
    ...
    <div class="container-sm h-100 main p-0 bg-dark">
        ...
        <ul class="pagination m-0 position-relative bottom-0 bg-dark">
            <li class="page-item <?php echo (($crpage == 1) ? "disabled not-selected" : "") ?>">
                <a class="page-link" tabindex="-1" href="/xlxx/khongche?page=<?php
                                                                            if ($crpage > 1) {
                                                                                echo $crpage - 1;
                                                                            } else {
                                                                                echo 1;
            ...
            ?>
                <li class="page-item <?php echo (($crpage == $i) ? "active" : "") ?>" <?php ?>><a class="page-link" href="/xlxx/khongche?page=<?php echo $i ?>"><?php echo $i ?></a></li>
            <?php
            }
            ?>
            <li class="page-item <?php echo (($crpage == $total_page) ? "disabled not-selected" : "") ?>"><a tabindex="-1" class="page-link" href="/xlxx/khongche?page=<?php echo ($crpage + 1) ?>">Next</a></li>
        </ul>
        <br><br>
        <div class="footer">
            <p>Xlxx &copy; 2021 |</p>
