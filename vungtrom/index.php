...
function paginates_4() {
    ...
    $categories= mysqli_query($conn,"SELECT * FROM movies where title like '%vụng trộm%' or title like '%Vụng trộm%' or title like '%ngoại tình%' or title like '%hàng xóm%' or title like '%bạn thân%' or title like '%khó tính%' or title like '%may mắn%' or title like '%đồng nghiệp%' or title like '%giám đốc%' or title like '%thư ký%' or title like '%dâm%' or title like '%sếp%' or title like '%trưởng phòng%' ORDER BY id");
...
$categories = mysqli_query($conn, "SELECT * FROM movies where title like '%vụng trộm%' or title like '%Vụng trộm%' or title like '%ngoại tình%' or title like '%hàng xóm%' or title like '%bạn thân%' or title like '%khó tính%' or title like '%may mắn%' or title like '%đồng nghiệp%' or title like '%giám đốc%' or title like '%thư ký%' or title like '%dâm%' or title like '%sếp%' or title like '%trưởng phòng%' limit $start,$limit");
?>
<!DOCTYPE html>
<html lang="vi">
<!-- head -->
...
<head>
    <meta charset="UTF-8">
    <title>Vụng trộm - XLXX</title>
    <base href="http://localhost/xlxx/vungtrom">
    ...
    <style>
        ...
        .nav-menu .row li.col:nth-of-type(4) {
            color: #ff0000 !important;
        }
        .nav-menu .row li.col:not(:nth-of-type(4)) {
            color: #fff !important;
        }
    ...
    <div class="container-sm h-100 main p-0 bg-dark">
        ...
        <ul class="pagination m-0 position-relative bottom-0 bg-dark">
            <li class="page-item <?php echo (($crpage == 1) ? "disabled not-selected" : "") ?>">
                <a class="page-link" tabindex="-1" href="/xlxx/vungtrom?page=<?php
                                                                            if ($crpage > 1) {
                                                                                echo $crpage - 1;
                                                                            } else {
                                                                                echo 1;
            ...
            ?>
                <li class="page-item <?php echo (($crpage == $i) ? "active" : "") ?>" <?php ?>><a class="page-link" href="/xlxx/vungtrom?page=<?php echo $i ?>"><?php echo $i ?></a></li>
            <?php
            }
            ?>
            <li class="page-item <?php echo (($crpage == $total_page) ? "disabled not-selected" : "") ?>"><a tabindex="-1" class="page-link" href="/xlxx/vungtrom?page=<?php echo ($crpage + 1) ?>">Next</a></li>
        </ul>
        <br><br>
        <div class="footer">
            <p>Xlxx &copy; 2021 |</p>
