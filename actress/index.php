...
if (isset($_GET['name'])) {
        $pst= $_GET['name'];
        $pst= ucfirst($pst);
        $pst= str_replace('_',' ',$pst);
        $pst= strval($pst);
        $conn = mysqli_connect("localhost", "root", "", "data_menu");
        $categories = mysqli_query($conn, "SELECT * FROM movies where actress_movies= '$pst'    ORDER BY id");
    ...
    $categories = mysqli_query($conn, "SELECT * FROM movies where actress_movies= '$pst' limit $start,$limit");
}
else {
    die('404 Not found');
}
...
<head>
    <meta charset="UTF-8">
    <base href="http://localhost/xlxx/actress">
    ...
    <div class="container-sm h-100 main p-0 bg-dark">
        ...
        <ul class="pagination m-0 position-relative bottom-0 bg-dark">
            <li class="page-item <?php echo (($crpage == 1) ? "disabled not-selected" : "") ?>">
                <a class="page-link" tabindex="-1" href="/xlxx/actress?name=<?php $_GET["name"];
                                                                                ?>">
                    Previous
                </a>
            </li>
            ...
            ?>
                <li class="page-item <?php echo (($crpage == $i) ? "active" : "") ?>" <?php ?>><a class="page-link" href="/xlxx/actress?name=<?php $_GET["name"];
                                                                                ?>"><?php echo $i ?></a></li>
            <?php
            }
            ?>
            <li class="page-item <?php echo (($crpage == $total_page) ? "disabled not-selected" : "") ?>"><a tabindex="-1" class="page-link" href="/xlxx/actress?name=<?php $_GET["name"];
                                                                                ?>">Next</a></li>
        </ul>
        <br><br>
        <div class="footer">
