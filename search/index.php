...
$ii=0;
        ...
        $query= mysqli_query($conn,"SELECT * FROM movies order by id asc");
        if(isset($_GET['result'])) {
            $rst= $_GET['result'];
            $rst_l= mb_strtolower($rst,'utf-8');
            while($rw= mysqli_fetch_array($query)) {
               if (str_contains(mb_strtolower($rw['title'],'utf-8'),$rst_l)==true or str_contains(mb_strtolower(convert_vi_to_en($rw['title']),'utf-8'),convert_vi_to_en($rst_l))==true) {
                //    echo $rw['title']."<br>";
                   $ii++;
               } 
        }
    ...
    // $categories= mysqli_query($conn,"SELECT * FROM movies where title like '%vụng trộm%' or title like '%Vụng trộm%' or title like '%ngoại tình%' or title like '%hàng xóm%' or title like '%bạn thân%' or title like '%khó tính%' or title like '%may mắn%' or title like '%mẹ%' or title like '%dượng%' or title like '%anh%' or title like '%dâm%' or title like '%bố%' or title like '%chồng%'or '%vợ%' or title like '%chị%' or title like '%dì%' or title like '%cháu%' or title like '%con%' and title not like '%đồng nghiệp%' and title not like '%sếp%' and title not like '%trưởng phòng%' and title not like '%nhân viên%' ORDER BY id");
...
<head>
    <title><?php echo $_GET['result'] ?> - XLXX</title>
    <meta charset="UTF-8">
    <base href="http://localhost/xlxx/search">
    ...
    <div class="container-sm h-100 main p-0 bg-dark">
        ...
        if(isset($_GET['result'])) {
            ...
            while($rw= mysqli_fetch_array($query)) {
               if (str_contains(mb_strtolower($rw['title'],'utf-8'),$rst_l)==true or str_contains(mb_strtolower(convert_vi_to_en($rw['title']),'utf-8'),convert_vi_to_en($rst_l))==true) {
                //    echo $rw['title']."<br>";
                ?>
                <a title="<?php echo $rw['title'] ?>" href="/xlxx/video?vd=<?php $cove = convert_vi_to_en($rw['title']);
                                                                            $dash = replaceAll($cove);
                                                                            $rmcm = str_replace(',', '', $dash);
                                                                            $fly = str_replace('.', '', $rmcm);
                                                                            echo $fly; ?>&id=<?php echo $rw['id'] ?>">
        ...
        <ul class="pagination m-0 position-relative bottom-0 bg-dark">
            <li class="page-item <?php echo (($crpage == 1) ? "disabled not-selected" : "") ?>">
                <a class="page-link" tabindex="-1" href="/xlxx/search?page=<?php
                                                                            if ($crpage > 1) {
                                                                                echo $crpage - 1;
                                                                            } else {
                                                                                echo 1;
            ...
            ?>
                <li class="page-item <?php echo (($crpage == $i) ? "active" : "") ?>" <?php ?>><a class="page-link" href="/xlxx/search?page=<?php echo $i ?>"><?php echo $i ?></a></li>
            <?php
            }
            ?>
            <li class="page-item <?php echo (($crpage == $total_page) ? "disabled not-selected" : "") ?>"><a tabindex="-1" class="page-link" href="/xlxx/search?page=<?php echo ($crpage + 1) ?>">Next</a></li>
        </ul>
        <br><br>
        <div class="footer">
            <p>Xlxx &copy; 2021 |</p>
            <p>Xlxx.com là web xem <a href="/xlxx/search">phim sex</a> cho người trên 18 tuổi, chỉ có một mục đích đó là giúp bạn giải trí và thỏa mãn sinh lý, nếu bạn dưới 18 tuổi vui lòng quay ra.</p>
